  ...
  try {
    ...
    if (!p.email || !p.name) {
      return {
        statusCode: 200,
        headers: cors(ORIGIN),
        body: JSON.stringify({ ok: false, error: 'Name + Email required' }),
      };
    }

    // Map form -> AppSheet row (rename keys to your exact column names if different)
    ...
    const row = {
      ...
      // If your column is Text (not EnumList), use: (Array.isArray(p.alarms) ? p.alarms.join(', ') : '')
    ...
    return { statusCode: 200, headers: cors(ORIGIN), body: JSON.stringify({ ok: true, result: json }) };
  } catch (err) {
